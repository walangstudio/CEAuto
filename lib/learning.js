...
function dispatchStats(agent, { passScore = 3 } = {}) {
  ...
  // provider is 1:1 with model in the ledger, so MAX() just carries it through
  // without splitting the per-model grouping (samples stay per model, not per
  // model+provider). Needed so a recommendation can route to the right provider.
  return db.prepare(`
    ...
    FROM (
      SELECT task_id, model, MAX(provider) AS provider, SUM(usd) AS usd
      FROM budget_ledger
...
/**
 * The cheapest (model, provider) whose success rate clears the bar with enough
 * samples — "cheapest that works", as a routable pair. Null when there isn't
 * enough signal yet (the runner then falls back to the configured model).
 *
 * Epsilon-greedy exploration (`epsilon` > 0): with probability `epsilon` the
 * policy returns an *abandoned* model instead of the exploit pick — one that is
 * under-sampled or has dropped below the success bar — so it can rediscover a
 * model that has since recovered or a newly-cheaper option, instead of only
 * self-correcting downward. Without it, once routing locks onto a winner the
 * alternatives stop accruing samples and can never climb back. The explore pick
 * is the least-sampled routable candidate (most neglected → most signal to gain;
 * ties break cheapest, since `stats` is cost-ascending) and is tagged
 * `explore: true`. It can fire even with no exploit pick yet, re-sampling a
 * below-bar/under-sampled model. Never-tried models aren't in `stats`, so this
 * re-samples the abandoned, not the unknown (cold start is out of scope). `rng`
 * is injectable so the decision is deterministic in tests. epsilon = 0 (default)
 * → exploit only, behaviour unchanged.
 */
function recommendDispatch(agent, { minSamples = 3, minSuccess = 0.6, epsilon = 0, rng = Math.random } = {}) {
  ...
  const exploit = good.length ? { model: good[0].model, provider: good[0].provider || null } : null;

  if (epsilon > 0 && rng() < epsilon) {
    // Explore only routable pairs (known provider) that aren't the exploit pick — a
    // partial route is ignored downstream, which would waste the exploration draw.
    const candidates = stats.filter(s =>
      s.provider && !(exploit && s.model === exploit.model && s.provider === exploit.provider)
    );
    if (candidates.length) {
      const pick = candidates.reduce((a, b) => (b.samples < a.samples ? b : a));
      return { model: pick.model, provider: pick.provider, explore: true };
    }
  }
  return exploit;
}
...
/**
 * Just the model string of the recommendation (for the ceo_insights table).
 * Null when there isn't enough signal yet.
 */
function recommendModel(agent, opts) {
  const rec = recommendDispatch(agent, opts);
