 ...
 *
 * Note: once auto-routing locks onto a winner the other models stop accruing
 * samples, so the policy can't *rediscover* a model it abandoned — but it
 * self-corrects downward, since a winner whose success rate later drops below
 * minSuccess falls out of `good` and routing reverts to the next cheapest or to
 * config. Good enough for an opt-in default-off dispatch policy.
 */
function recommendDispatch(agent, { minSamples = 3, minSuccess = 0.6 } = {}) {
  ...
  return { model: good[0].model, provider: good[0].provider || null };
}
