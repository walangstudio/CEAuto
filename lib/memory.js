...
function init(dbPath) {
  ...
  db.exec(`
    ...
    -- Append-only event log (Pillar 4). The autoincrement id is the total order;
    -- state is replayable by folding these in id order. Never UPDATE or DELETE.
...
function migrate(d) {
  addColumns(d, 'tasks', { depends_on: 'TEXT', parent_id: 'TEXT', plan: 'INTEGER NOT NULL DEFAULT 0' });
  addColumns(d, 'approvals', { quorum: 'INTEGER NOT NULL DEFAULT 1', votes: "TEXT DEFAULT '{}'" });
}

// NOTE: SQLite's ALTER TABLE ADD COLUMN only allows NOT NULL with a CONSTANT
// DEFAULT (it backfills existing rows). A NOT NULL column without a default — or
// with a non-constant default — throws on existing DBs. Keep new columns either
// nullable or NOT NULL DEFAULT <const>.
function addColumns(d, table, want) {
