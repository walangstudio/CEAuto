...
async function mcpTool(ctx, deps = {}) {
  ...
  // Optional command allowlist (basenames). Empty/absent = allow (the command is
  // operator config, not task data); a non-empty list locks it down like shell.
  const allowlist = deps.mcpAllowlist || [];
  if (allowlist.length && !allowlist.map(a => path.basename(String(a))).includes(path.basename(String(command)))) {
    throw new Error(`mcp-tool executor: "${command}" is not in the allowlist`);
  }
