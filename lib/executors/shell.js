...
const MAX_OUTPUT = 5 * 1024 * 1024; // 5 MiB cap on stdout/stderr — guards against a runaway child OOMing the daemon.
  ...
  return await new Promise((resolve, reject) => {
    ...
    child.stdout.on('data', d => {
      ...
      if (out.length > MAX_OUTPUT) {
        ...
        finish(() => reject(new Error(`shell executor: "${basename(command)}" exceeded ${MAX_OUTPUT} bytes of output`)));
      }
    });
    ...
    timer = setTimeout(() => {
      try { child.kill(); } catch { /* gone */ }
      finish(() => reject(new Error(`shell executor: timed out after ${timeoutMs}ms`)));
    }, timeoutMs);
    child.on('close', (code) => {
      if (code !== 0) {
        finish(() => reject(new Error(`shell executor: "${basename(command)}" exited ${code}: ${err.slice(-200)}`)));
        return;
      }
      finish(() => resolve({
        text: out,
