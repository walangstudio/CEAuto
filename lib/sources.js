...
/**
 * Turn a signal into a backlog task + a `source.<type>` event. `needsApproval`
 * defaults true: an externally-injected task must not run an agent's executor
 * autonomously without a human ack (the daemon sets sources.*.auto_approve to opt out).
 */
function ingest(signal, { needsApproval = true } = {}) {
 ...
 */
function watchFiles({ paths = [], agent = 'ops', on = ['add', 'change'], debounceMs = 1000, needsApproval = true } = {}, deps = {}) {
  ...
  for (const ev of on) {
    watcher.on(ev, (filePath) => {
      try {
        ...
        last.set(key, t);
        ingest(fromFileChange({ path: filePath, event: ev }, { agent }), { needsApproval });
      } catch {
 ...
 */
function webhookHandler({ secret = '', agent = 'ops', map = {}, agents = DEFAULT_AGENTS, needsApproval = true } = {}) {
  return (req, res, { body }) => {
    ...
    const task = ingest(signal, { needsApproval });
