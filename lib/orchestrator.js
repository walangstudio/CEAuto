 ...
 */
async function run(name, goal, params, workspace, mem, deps = {}) {
  // `name` becomes a file path — reject separators/`..` so it can't read an
  // arbitrary .md outside workflows/.
  if (/[\\/]/.test(String(name)) || String(name).includes('..')) {
  ...
  const sessionId = deps.sessionId || 'workflow';
  const results = {};
  const log = [`# Workflow: ${name}`, `**Goal:** ${goal}`, `**Started:** ${new Date().toISOString()}`, ''];

  // Global kill switch + budget pause: workflow steps spend real tokens, so gate
  // + record them like any other runtime (ceo_metrics/budget see the spend and a
  // cap actually stops the run).
  if (isHalted(workspace)) {
    return `Workflow "${name}" not run — halted (comms/STOP present).`;
  }
  if (budget.isPaused()) {
    return `Workflow "${name}" not run — autonomous spend is paused (${budget.pauseReason() || 'budget hold'}).`;
  }

  for (const step of steps) {
    // Kill switch is re-checked per step (like the budget gate) so a STOP that
    // lands mid-workflow halts the remaining steps, not just the first.
    if (isHalted(workspace)) {
      log.push('**Halted:** comms/STOP present — remaining steps skipped.');
      log.push('');
      break;
    }
    ...
    try {
      ...
      const est = estimateTokens(`${agentSpec}${context}${task}`);
      const affordable = budget.canSpend(agent, est, { sessionId });
      if (!affordable.ok) {
        log.push(`**Status:** ⏸ Skipped — budget: ${affordable.reason}`);
        log.push('');
        results[id] = `SKIPPED (budget): ${affordable.reason}`;
        break;
      }
      const { text: output, usage } = await dispatch(agent, agentSpec, task, context);
      results[id] = output;
      budget.record({
        agent,
        task_id: `${name}/${id}`,
...
module.exports = { run, loadAgentSpec, readContextFiles };
