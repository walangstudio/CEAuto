...
function renderBacklog(rows) {
  const header =
    '# Backlog\n\n| ID | Task | Agent | Priority | Deadline | Depends On |\n|----|------|-------|----------|----------|------------|\n';
  const body = rows
    .map(t => {
      const deps = scheduler.parseDeps(t);
      return `| ${t.id} | ${t.title} | ${t.agent || '—'} | ${t.priority || 'P2'} | ${t.deadline || 'TBD'} | ${deps.length ? deps.join(', ') : '—'} |`;
    })
    .join('\n');
  return header + body + '\n';
}
