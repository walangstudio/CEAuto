...
// fetch with a hard abort, so a hung endpoint can't run past the runner's
// Promise.race (which abandons but never cancels the request, letting it keep
// spending provider-side and then get retried).
async function fetchWithTimeout(url, opts = {}, timeoutMs = DEFAULT_HTTP_TIMEOUT_MS) {
...
// A base_url carries the API key in an Authorization header, so plain http would
// leak it. Allow http only for loopback (local proxies like Ollama/LM Studio).
function assertSafeBaseUrl(baseUrl) {
  ...
  // WHATWG URL returns the IPv6 host bracketed, e.g. '[::1]'.
  const loopback = u.hostname === 'localhost' || u.hostname === '127.0.0.1' || u.hostname === '[::1]' || u.hostname === '::1';
  if (u.protocol !== 'https:' && !loopback) {
    throw new Error(`refusing to send the API key over ${u.protocol} to ${u.hostname} — use https or a loopback host`);
  }
}
...
async function dispatchOpenAI(model, systemPrompt, task, context) {
  ...
  const res = await fetchWithTimeout(`${baseUrl}/chat/completions`, {
    ...
    body: JSON.stringify({
      // No max_tokens: real OpenAI reasoning models (o1/o3/gpt-5) reject it and
      // require max_completion_tokens instead, while OpenAI-compatible endpoints
      // vary — omitting it keeps every backend working, as before.
      model,
      messages: [
...
async function dispatchGoogle(model, systemPrompt, task, context) {
  ...
  try {
    ...
    throw new Error('google provider needs an optional dependency: run `npm install @google/generative-ai` (not bundled by default)');
  }
