...
async function runTask(taskId, deps = {}) {
  ...
  // Governance gate: a task flagged needs_approval may not run until approved.
  if (task.needs_approval && !approvals.isApproved('task', taskId)) {
    if (!approvals.hasPending('task', taskId)) {
      approvals.request({ kind: 'task', ref_id: taskId, summary: `Approval required to run: ${task.title}`, detail: { agent: task.agent } });
      approvals.renderApprovals(workspace);
    }
  ...
  const est = estimateTokens(`${agentSpec}${context}${taskText}`);

  // Resolve how this agent runs. Precedence: explicit dep → settings by_agent →
  // the agent's org role executor → settings default → llm.
  const execCfg = (deps.settings && deps.settings.executors) || {};
  const executorId = deps.executorId || execCfg.by_agent?.[agent] || org.executorFor(agent) || execCfg.default || 'llm';
  ...
  const execDeps = { dispatch, shellAllowlist };

  let lastErr = null;
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    // Budget gate re-checked before EVERY dispatch so retries can't blow the cap.
    ...
    try {
      const out = await withTimeout(
        executors.execute(executorId, { agent, agentSpec, task: taskText, context, params: execParams }, execDeps),
        timeoutMs,
      ...
      if (after.status === 'done') {
        // Inter-agent delegation (Pillar 5): only act on the directive once the
        // task is genuinely done (not on an eval requeue), so we don't spawn
        // children for work that's about to be redone. Isolated in its own
        // try/catch — delegation is a best-effort side effect of a DONE task, so
        // a child-spawn failure must never bubble into the dispatch-retry loop
        // (which would re-run the LLM and could double-spawn children).
        let delegated = [];
        try {
      ...
      if (deps.hooks && after.status === 'blocked') {
