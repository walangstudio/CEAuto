...
// Runtimes that cost $0 of LLM API spend. Pricing their proxy token counts at
// pricing.default would accrue phantom USD against the global USD cap.
//
//   no LLM at all: a local command (shell), an external worker (webhook), or
//   another MCP server (mcp).
//
//   subscription-billed CLIs: `claude -p` and `codex exec` bill the operator's
//   Claude / ChatGPT PLAN, not per-token API credits. This is keyed on the PROVIDER
//   (a hardcoded constant in each executor), never on the model/binary name, which
//   is operator-config and could be renamed into or out of a price. The executors
//   also strip ANTHROPIC_API_KEY/OPENAI_API_KEY from the child env, so the CLI
//   CANNOT fall back to API billing — that is what makes $0 true rather than
//   assumed. Opting in via `passApiKeys: true` means you should set real prices.
//
// `composite` stays EXCLUDED — it can chain a real `llm` step, so it is priced
// conservatively via pricing.default. Tokens are still metered for everything here.
const NON_LLM_PROVIDERS = new Set(['shell', 'webhook', 'mcp', 'claude-code', 'codex']);
